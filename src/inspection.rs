            ...
            for deployment in deployments {
                if let Value::Object(dep_map) = deployment {
                    ...
                    if !search.is_empty() {
                        ...
                        let dep_name = crate::value::str(dep_map.get("name").unwrap_or(&Value::Null));
                    ...
                    final_row.insert("key".to_string(), app.get("key").unwrap_or(&Value::Null).clone());
                    final_row.insert("type".to_string(), app.get("type").unwrap_or(&Value::Null).clone());
...
mod tests {
    ...
    fn test_deployed_app_rows_filtering() {
        ...
        assert_eq!(rows[0].get("key").map(|v| v.as_str()).flatten(), Some("app1"));
    }
